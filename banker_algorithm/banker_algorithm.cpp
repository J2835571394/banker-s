...
void input() {//input the information
	...
	for (int i = 0; i < p; i++)
	{
		...
		for (int j = 0; j < r; j++) cin >> maxx[i][j];
		cout << "plz input the allocate vertex:" << endl;
		for (int j = 0; j < r; j++) cin >> allocate[i][j];
		for (int j = 0; j < r; j++) need[i][j]=maxx[i][j]-allocate[i][j];
	}
	cout << "plz input the current avaliable vertex:" << endl;
	for (int j = 0; j < r; j++) cin >> current[j];
}

int Compare(int m[], int n[]) {
	for (int i = 0; i < r; i++) if (m[i] > n[i]) return 0;
	return 1;
}
...
int ifsafe() {
	...
	for (int i = 0; i < r; i++) work[i] = current[i];
	for (int k = 0; k < p; k++) {
		for (int i = 0; i < p; i++) {
			if (finish[i] == 0) {
	...
	for (int i = 0; i < p; i++) if (finish[i] == 0) return 1;
	return 0;
}
...
void requestest(int n) {
	...
	if (Compare(request, current)&&Compare(request, need[n - 1])) {
		...
		if (ifsafe()) cout << "execute the request successfully." << endl;
		else {
...
int main() {
	...
	if (ifsafe()) {
		...
		for (int i = 0; i < p; i++) cout << "p" << safesequence[i]<<" ";
		cout << endl;
		while(1){
	...
	else cout << "the state is unsafe! plz modify the value." << endl;
	cout << "All the processes are finished." << endl;
	return 0;
}
